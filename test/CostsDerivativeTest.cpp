...
void configureCosts(const VariablesLabeller& stateVariables, const VariablesLabeller& controlVariables,
    ...
    for (size_t i = 0; i < leftPositions.size(); ++i) {
        ...
        std::shared_ptr<ForceRatioCost> forceRatiosCost = std::make_shared<ForceRatioCost>(stateVariables, controlVariables, "Left", i);
        forceRatiosCost->setDesiredRatio(iDynTree::getRandomDouble());
        ok = ocProblem.addLagrangeTerm(1.0, forceRatiosCost);
        ASSERT_IS_TRUE(ok);
    ...
    for (size_t i = 0; i < rightPositions.size(); ++i) {
        ...
        std::shared_ptr<ForceRatioCost> forceRatiosCost = std::make_shared<ForceRatioCost>(stateVariables, controlVariables, "Right", i);
        forceRatiosCost->setDesiredRatio(iDynTree::getRandomDouble());
        ok = ocProblem.addLagrangeTerm(1.0, forceRatiosCost);
        ASSERT_IS_TRUE(ok);
...
//    ASSERT_IS_TRUE(ok);
    ...
    iDynTree::VectorDynSize weights(3);
    iDynTree::toEigen(weights).setConstant(15.0);
    meanPositionCost->setTimeVaryingWeight(std::make_shared<iDynTree::optimalcontrol::TimeInvariantVector>(weights));
...
//    checkStaticForcesJacobian(0.0, stateVector, controlVector, 0.001, staticTorquesPtr, 0);
    ...
    iDynTree::toEigen(stateVector) *= 0.5;
    checkCostsHessian(0.0, stateVector, controlVector, 0.0001, ocProblem);
    iDynTree::getRandomVector(stateVector);
    iDynTree::getRandomVector(controlVector);
    checkCostsHessian(1.0, stateVector, controlVector, 0.0001, ocProblem);
